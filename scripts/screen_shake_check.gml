...
if (shaking == true)                 
{
    if (shake > 0)                    // if there is still time to screen shake, then shake away!
    {
                                            // NOTE: make shake decrement with time? divide it by shake_count/shake_count_max?
        shake = random_range(-shake, shake);    // assigns the actual shake value to a random number within a shake range.
        shake -= 1;                   // decrement shake count
    }
